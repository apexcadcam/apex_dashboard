#!/usr/bin/env python3
"""
Generate 20 Template Dashboards
Creates all necessary files for template dashboards that can be activated from UI
"""

import os
from pathlib import Path

def generate_template_dashboard(template_number):
    """Generate a single template dashboard"""
    
    route = f"template_dashboard_{template_number:02d}"
    title = f"Template Dashboard {template_number:02d}"
...
def get_dashboard_data(company=None, period="This Month", from_date=None, to_date=None):
    ...
    # Get dashboard info to determine category
    dashboard = frappe.get_doc("Apex Dashboard", "{route}")
    category = dashboard.category if dashboard else "General"
...
'''
 ...
 "title": "{title}"
    ...
    new TemplateDashboard{template_number:02d}(wrapper);
}};

class TemplateDashboard{template_number:02d} {{
    constructor(wrapper) {{
        ...
        this.load_dashboard_title();
        this.setup_filters();
        this.load_data();
    }}
    
    load_dashboard_title() {{
        // Get actual dashboard title from database
        frappe.call({{
            method: 'frappe.client.get_value',
            args: {{
                doctype: 'Apex Dashboard',
                filters: {{ route: '{route}' }},
                fieldname: ['title', 'icon']
            }},
            callback: (r) => {{
                if (r.message) {{
                    let title = r.message.title || '{title}';
                    let icon = r.message.icon || '';
                    this.page.set_title(icon ? `${{icon}} ${{title}}` : title);
                }}
            }}
        }});
    }}
...
def main():
    print("="*70)
    print("GENERATING 20 TEMPLATE DASHBOARDS")
    print("="*70)
    print()
    
    created = []
    for i in range(1, 21):
        route = generate_template_dashboard(i)
        created.append(route)
        print(f"✓ Created: {route}")
    
    print()
    print("="*70)
    print(f"✅ GENERATED {len(created)} TEMPLATE DASHBOARDS!")
    print("="*70)
    print()
    print("Next steps:")
    print("1. Run: python3 install_templates.py")
    print("2. Run: bench migrate")
    print("3. Run: bench clear-cache && bench restart")
    print()
