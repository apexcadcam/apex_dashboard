frappe.pages['test'].on_page_load = function (wrapper) {
    ...
    // Load CSS
    frappe.require('/assets/apex_dashboard/page/test/test.css');
...
<div class="dashboard-template">
    <div class="dashboard-header">
        <div class="header-content">
            <h1>test Overview</h1>
            <p class="subtitle">Track your equity</p>
        </div>
...
class DashboardTemplate {
    ...
    fetch_data() {
        ...
        frappe.call({
            method: 'apex_dashboard.apex_dashboard.page.test.test.get_dashboard_data',
            args: args,
            callback: (r) => {
    ...
    render(data) {
        ...
        data.groups.forEach(group => {
            ...
            const themeClass = this.get_theme_class(group.name);

            let style = "";
            if (group.color) {
                style = `background: linear-gradient(135deg, ${group.color}66 0%, rgba(255, 255, 255, 0.1) 100%); border-color: ${group.color}80;`;
            }
            ...
            const cardHtml = `
                <div class="category-card ${themeClass}" style="${style}">
